...
function pathFromArchetype(
  ...
  if (!primaryTitle || primaryTitle === a.title) {
    why =
      ...
      locationSentence(ans.q5, a.title);
  } else {
...
function computePlan(ans: LocalAnswers): AssessmentPlan {
  ...
  const adjacent = (adj.slice(0, 2) as string[]).map((k) =>
    pathFromArchetype(ARCHETYPES[k], ans, primary.title),
  );
  ...
  // Urgency override — unless the blocker is not knowing what roles exist:
  // then phase 1 leans harder on directory exploration than on applying.
  if (ans.q10 === "I don't know what roles exist") {
    days0to30.push(
      urgent
  ...
  // Phase 2
  days30to60.push(interviewTask(ans.q5, ans.q10 === "No network in this field"));
