...
// ─── Downgrade Envoy → Compass at period end ────────────────────────────────
...
export const downgradeToCompass = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    try {
      ...
      // Match the current cadence so users keep their billing rhythm.
      const current = await stripe.prices.retrieve(sub.price_id as string);
      const isAnnual = current?.recurring?.interval === "year";
      const lookupKey = isAnnual ? "compass_annual" : "compass_monthly";
      const prices = await stripe.prices.list({ lookup_keys: [lookupKey] });
      if (!prices.data.length) return { error: "Compass price not configured" };
      const compassPriceId = prices.data[0].id;
      ...
      // Schedule the downgrade at period end with no proration so it doesn't
      // double-charge or refund mid-cycle.
      await stripe.subscriptions.update(sub.stripe_subscription_id as string, {
        items: [{ id: itemId, price: compassPriceId }],
        proration_behavior: "none",
        billing_cycle_anchor: "unchanged",
        metadata: { tier: "compass", cadence: isAnnual ? "annual" : "monthly" },
      } as any);
      return { ok: true as const };
    } catch (error) {
      const { getStripeErrorMessage } = await import("@/lib/stripe.server");
...
// ─── Employer credits checkout (one-time) ───────────────────────────────────
...
export const createEmployerCreditsCheckout = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<CheckoutResult> => {
    try {
      ...
      const prices = await stripe.prices.list({ lookup_keys: [lookupKey] });
      if (!prices.data.length) return { error: "Credit price not configured" };
      const stripePrice = prices.data[0];
